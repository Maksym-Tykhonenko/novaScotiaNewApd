...
const MegaQuiz = ({navigation}) => {
  ...
  useEffect(() => {
    if (!isWelcomeModalVisible) loadRandomQuestion();
  }, [isWelcomeModalVisible]);

  const loadRandomQuestion = () => {
    const quizKeys = Object.keys(quizzes);
    const randomKey = quizKeys[Math.floor(Math.random() * quizKeys.length)];
    const randomQuiz = quizzes[randomKey];
    const randomQuestion =
      randomQuiz[Math.floor(Math.random() * randomQuiz.length)];

    if (randomQuestion.type === 'matching') {
      setAvailableItems(randomQuestion.items);
      setMatchedItems(Array(randomQuestion.items.length).fill(null));
      setCurrentItems(
        shuffleArray(randomQuestion.items.slice(0, itemsPerBatch)),
      );
    }
  ...
  const handleDrop = (dropIndex, item) => {
    ...
    if (
      ...
      setTimeout(() => {
        ...
        setCurrentItems(shuffleArray(nextBatch));
      }, 500);
    }
  };

  const submitMatchingAnswer = () => {
    const correct = matchedItems.every(
      (item, index) => item && item.text === currentQuestion.items[index].text,
    );
  ...
  const saveScore = async () => {
    try {
      ...
      const scoreToSave = Math.floor(score / 5);
      const totalCoins =
        (existingCoins ? parseInt(existingCoins, 10) : 0) + scoreToSave;
        ...
        <View style={styles.container}>
          ...
          {isWelcomeModalVisible ? (
            <Modal
              ...
              <View style={styles.modalContent}>
                <Text style={styles.welcomeText}>
                  Welcome to Mega Quiz! Ready to explore Nova Scotia?
                </Text>
                <TouchableOpacity
            ...
            <Modal
              ...
              <View style={styles.modalContent}>
                <Text style={styles.welcomeText}>
                  Hardcore Quiz mixes all topics and modes. You have only one
                  life. Good luck!
                </Text>
                <TouchableOpacity
                  style={styles.okButton}
                  onPress={() => setRulesModalVisible(false)}>
                  <Text style={styles.okButtonText}>Start</Text>
                </TouchableOpacity>
              </View>
            </Modal>
          ) : (
